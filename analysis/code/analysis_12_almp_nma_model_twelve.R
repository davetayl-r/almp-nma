...
almp_nma_model_twelve_data <- almp_nma_additive_model_data |>
...
almp_nma_model_twelve_formula <- bf(
  delta | se(delta_se) ~
    0 +
...
possible_prior_names <- get_prior(
  almp_nma_model_twelve_formula,
  data = almp_nma_model_twelve_data,
  family = gaussian()
)
...
almp_nma_model_twelve <- brm(
  formula = almp_nma_model_twelve_formula,
  data = almp_nma_model_twelve_data,
...
# inspect the results
summary(almp_nma_model_twelve)

# inspect diagnostic plots
#plot(almp_nma_model_twelve)

pp_check(almp_nma_model_twelve, type = "dens_overlay", ndraws = 200)
pp_check(almp_nma_model_twelve, type = "intervals")

# examine influence
#almp_nma_model_twelve_loo <- add_criterion(
#  almp_nma_model_twelve,
#  "loo",
#  reloo = FALSE
#)
...
saveRDS(
  ...
  "./visualisation/inputs/prototype_models/almp_nma_model_twelve_component_summary.RDS"
)
