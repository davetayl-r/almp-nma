...
# Author: David Taylor                                                                       #
# Date: 18/09/2025                                                                           #
# Purpose: NMA model #3 — introduce study quality (as well as study design)                  #
#============================================================================================#
...
almp_nma_model_three_data <- almp_nma_additive_model_data |>
...
almp_nma_model_three_formula <- bf(
  delta | se(delta_se) ~
...
almp_nma_model_three <- brm(
  formula = almp_nma_model_three_formula,
  data = almp_nma_model_three_data,
  prior = almp_nma_model_three_priors,
  family = gaussian(),
  chains = 2,
  iter = 4000,
  warmup = 2000,
...
# inspect the results
summary(almp_nma_model_three)

# inspect diagnostic plots
#plot(almp_nma_model_three)

pp_check(almp_nma_model_three)
...
almp_nma_model_three_tau_draws <- almp_nma_model_three |>
  gather_draws(
    `sd_.*`,
    regex = TRUE
  ) |>
...
almp_nma_model_three_component_draws <- almp_nma_model_three |>
  gather_draws(
    `b_.*:comp_.*`,
    regex = TRUE
  ) |>
...
almp_nma_model_three_component_summary <- almp_nma_model_three_component_draws |>
  group_by(
    outcome,
    component
  ) |>
...
saveRDS(
  almp_nma_model_three_component_summary,
  "./visualisation/inputs/prototype_models/almp_nma_model_three_component_summary.RDS"
)

saveRDS(
  almp_nma_model_three_component_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_three_component_draws.RDS"
)

saveRDS(
  almp_nma_model_three_tau_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_three_tau_draws.RDS"
)
