...
# Author: David Taylor                                                                       #
# Date: 24/09/2025                                                                           #
# Purpose: NMA model #13 — is this it? I hope so                                             #
#============================================================================================#
...
almp_nma_model_thirteen_data <- almp_nma_additive_model_data |>
...
almp_nma_model_thirteen_formula <- bf(
  delta | se(delta_se) ~
    0 +
...
possible_prior_names <- get_prior(
  almp_nma_model_thirteen_formula,
  data = almp_nma_model_thirteen_data,
  family = gaussian()
)
...
almp_nma_model_thirteen_priors <- do.call(
  c,
  c(
    prior_base,
...
almp_nma_model_thirteen <- brm(
  formula = almp_nma_model_thirteen_formula,
  data = almp_nma_model_thirteen_data,
  prior = almp_nma_model_thirteen_priors,
  family = gaussian(),
  chains = 4,
  iter = 2000,
  warmup = 1000,
...
# inspect the results
summary(almp_nma_model_thirteen)

# inspect diagnostic plots
#plot(almp_nma_model_thirteen)

pp_check(almp_nma_model_thirteen, type = "dens_overlay", ndraws = 200)
pp_check(almp_nma_model_thirteen, type = "intervals")

# examine influence
#almp_nma_model_thirteen_loo <- add_criterion(
#  almp_nma_model_thirteen,
#  "loo",
#  reloo = FALSE
#)
#summary(almp_nma_model_thirteen_loo$criteria$loo) # check Pareto k
...
almp_nma_model_thirteen_draws <- as_draws_df(almp_nma_model_thirteen)
...
almp_nma_model_thirteen_tau_component_design_draws <- almp_nma_model_thirteen_draws |>
...
almp_nma_model_thirteen_tau_component_design_summary <- almp_nma_model_thirteen_tau_component_design_draws |>
...
#almp_nma_model_thirteen_tau_design_draws <- almp_nma_model_thirteen_draws %>%
...
#almp_nma_model_thirteen_tau_design_summary <- almp_nma_model_thirteen_tau_design_draws %>%
#  group_by(design) %>%
#  summarise(
#    tau_med = median(tau),
#    .lower = quantile(tau, 0.05),
...
saveRDS(
  almp_nma_model_thirteen_tau_component_design_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_thirteen_tau_component_design_draws.RDS"
)

saveRDS(
  almp_nma_model_thirteen_tau_component_design_summary,
  "./visualisation/inputs/prototype_models/almp_nma_model_thirteen_tau_component_design_summary.RDS"
)

#saveRDS(
#  almp_nma_model_thirteen_tau_design_draws,
#  "./visualisation/inputs/prototype_models/almp_nma_model_thirteen_tau_study_design_draws.RDS"
#)

#saveRDS(
#  almp_nma_model_thirteen_tau_design_summary,
#  "./visualisation/inputs/prototype_models/almp_nma_model_thirteen_tau_study_design_summary.RDS"
#)
...
almp_nma_model_thirteen_component_draws_flagged <- almp_nma_model_thirteen_draws |>
...
almp_nma_model_thirteen_component_draws <- almp_nma_model_thirteen_draws |>
  select(.draw, matches("^b_outcome.*:comp_")) |>
  pivot_longer(
    cols = -.draw,
    names_to = "param",
...
almp_nma_model_thirteen_component_draws_combined <- left_join(
  almp_nma_model_thirteen_component_draws,
  almp_nma_model_thirteen_component_draws_flagged,
  by = c(
    "outcome",
    "component"
  )
...
almp_nma_model_thirteen_component_summary <- almp_nma_model_thirteen_component_draws_combined |>
  group_by(
    outcome,
    component,
...
saveRDS(
  almp_nma_model_thirteen_component_summary,
  "./visualisation/inputs/prototype_models/almp_nma_model_thirteen_component_summary.RDS"
)

saveRDS(
  almp_nma_model_thirteen_component_draws_combined,
  "./visualisation/inputs/prototype_models/almp_nma_model_thirteen_component_draws.RDS"
)
