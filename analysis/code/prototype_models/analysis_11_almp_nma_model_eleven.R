...
# Author: David Taylor                                                                       #
# Date: 22/09/2025                                                                           #
# Purpose: NMA model #10 — change up approach                                                #
#============================================================================================#
...
almp_nma_model_eleven_data_x <- almp_nma_additive_model_data |>
  group_by(outcome) |>
  tally() |>
  arrange(n)

almp_nma_model_eleven_data <- almp_nma_additive_model_data |>
...
almp_nma_model_eleven_formula <- bf(
  delta | se(delta_se) ~
    0 +
...
possible_prior_names <- get_prior(
  almp_nma_model_eleven_formula,
  data = almp_nma_model_eleven_data,
  family = gaussian()
)
...
almp_nma_model_eleven_priors <- do.call(
  c,
  c(
    prior_base,
    prior_outcome,
...
almp_nma_model_eleven <- brm(
  formula = almp_nma_model_eleven_formula,
  data = almp_nma_model_eleven_data,
  prior = almp_nma_model_eleven_priors,
  family = gaussian(),
  chains = 2,
  iter = 3000,
  warmup = 1000,
...
# inspect the results
summary(almp_nma_model_eleven)

# inspect diagnostic plots
#plot(almp_nma_model_eleven)

pp_check(almp_nma_model_eleven, type = "dens_overlay", ndraws = 200)
pp_check(almp_nma_model_eleven, type = "intervals")

# examine influence
#almp_nma_model_eleven_loo <- add_criterion(
#  almp_nma_model_eleven,
#  "loo",
#  reloo = FALSE
#)
#summary(almp_nma_model_eleven_loo$criteria$loo) # check Pareto k
...
almp_nma_model_eleven_tau_component_draws <- almp_nma_model_eleven |>
...
almp_nma_model_eleven_tau_study_design_draws <- almp_nma_model_eleven |>
  # extract tau for study-level REs by outcome × design
  gather_draws(
...
component_variables <- grep(
  "^comp_",
  names(almp_nma_model_eleven$data),
  value = TRUE
)
outcome_levels <- levels(almp_nma_model_eleven$data$outcome)
almp_nma_model_eleven$data$study_design_type <- factor(
  almp_nma_model_eleven$data$study_design_type
)
design_levels <- levels(almp_nma_model_eleven$data$study_design_type)
...
grid_base <- tibble(
  ...
  delta = 0,
  delta_se = mean(almp_nma_model_eleven$data$delta_se, na.rm = TRUE)
) |>
  as.data.frame()
...
almp_nma_model_eleven_component_draws <- map_dfr(
  component_variables,
...
almp_nma_model_eleven_component_summary <- almp_nma_model_eleven_component_draws |>
  group_by(
    outcome,
    component
  ) |>
...
saveRDS(
  almp_nma_model_eleven_component_summary,
  "./visualisation/inputs/prototype_models/almp_nma_model_eleven_component_summary.RDS"
)

saveRDS(
  almp_nma_model_eleven_component_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_eleven_component_draws.RDS"
)

saveRDS(
  almp_nma_model_eleven_tau_component_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_eleven_tau_component_draws.RDS"
)

saveRDS(
  almp_nma_model_eleven_tau_study_design_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_eleven_tau_study_design_draws.RDS"
)
