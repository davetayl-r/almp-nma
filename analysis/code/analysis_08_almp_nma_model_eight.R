...
# Author: David Taylor                                                                       #
# Date: 19/09/2025                                                                           #
# Purpose: NMA model #8 — add sex and age + introduce variation by USA                       #
#============================================================================================#
...
almp_nma_model_eight_data <- almp_nma_additive_model_data |>
  ...
  mutate(
    study_context = interaction(
      study_design_type,
      ifelse(united_states == 1, "united_states", "non_united_states"),
      sep = "_",
      drop = TRUE
    )
  )
...
almp_nma_model_eight_formula <- bf(
  delta | se(delta_se) ~
    ...
    0 +
      ...
      # additive study-design adjustments using united states as a baseline
      united_states +
      outcome_domain:united_states +
      # add a meaningful comment here
      prop_female_centred +
      ...
      # random effects
      (0 + outcome || gr(study, by = study_context)) +
      (1 | gr(study, by = study_context))
)
...
possible_prior_names <- get_prior(
  almp_nma_model_eight_formula,
  data = almp_nma_model_eight_data,
  family = gaussian()
)
...
# global time slope (per year away from 24m)
...
# Main US effect
prior_united_states_main <- make_coef_priors(
  possible_prior_names,
  "^united_states$",
  0.12
)

# Domain-varying US offsets
prior_united_states_domain <- make_coef_priors(
  possible_prior_names,
  "^(outcome_domain.*:united_states|united_states:outcome_domain[A-Z].*)",
  0.12
)

# main slopes (already centered; 1 unit = 1 SD if you z-scored)
...
# Combine & flatten
almp_nma_model_eight_priors <- do.call(
  c,
  c(
    prior_base,
    ...
    prior_united_states_main,
    prior_united_states_domain,
    prior_sex_main,
    prior_age_main,
...
almp_nma_model_eight <- brm(
  formula = almp_nma_model_eight_formula,
  data = almp_nma_model_eight_data,
  prior = almp_nma_model_eight_priors,
  family = gaussian(),
  chains = 2,
  iter = 3000,
  warmup = 1500,
...
# inspect the results
summary(almp_nma_model_eight)

# inspect diagnostic plots
#plot(almp_nma_model_eight)

pp_check(almp_nma_model_eight)
...
almp_nma_model_eight_component_draws <- almp_nma_model_eight |>
  gather_draws(
    `b_.*:comp_.*`,
    regex = TRUE
  ) |>
...
almp_nma_model_eight_component_summary <- almp_nma_model_eight_component_draws |>
  group_by(
    outcome,
    component
  ) |>
...
saveRDS(
  almp_nma_model_eight_component_summary,
  "./visualisation/inputs/prototype_models/almp_nma_model_eight_component_summary.RDS"
)

saveRDS(
  almp_nma_model_eight_component_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_eight_component_draws.RDS"
)
