...
almp_nma_model_two_data <- almp_nma_additive_model_data |>
...
almp_nma_model_two <- brm(
  ...
  data = almp_nma_model_two_data,
  prior = almp_nma_model_two_priors,
  family = gaussian(),
  chains = 2,
  iter = 4000,
  warmup = 2000,
  ...
  threads = threading(2),
  backend = "cmdstanr",
  init = "random", # safer inits for SDs
...
# inspect the results
summary(almp_nma_model_two)

# inspect diagnostic plots
#plot(almp_nma_model_two)

pp_check(almp_nma_model_two)
...
almp_nma_model_two_tau_draws <- almp_nma_model_two |>
  gather_draws(
    `sd_.*`,
    regex = TRUE
  ) |>
...
almp_nma_model_two_component_draws <- almp_nma_model_two |>
  gather_draws(
    `b_.*:comp_.*`,
    regex = TRUE
  ) |>
...
almp_nma_model_two_component_summary <- almp_nma_model_two_component_draws |>
  group_by(
    outcome,
    component
  ) |>
...
saveRDS(
  almp_nma_model_two_component_summary,
  "./visualisation/inputs/prototype_models/almp_nma_model_two_component_summary.RDS"
)

saveRDS(
  almp_nma_model_two_component_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_two_component_draws.RDS"
)

saveRDS(
  almp_nma_model_two_tau_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_two_tau_draws.RDS"
)
