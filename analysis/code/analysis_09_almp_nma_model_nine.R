...
almp_nma_model_nine_data <- almp_nma_additive_model_data |>
...
almp_nma_model_nine_formula <- bf(
  delta | se(delta_se) ~
...
possible_prior_names <- get_prior(
  almp_nma_model_nine_formula,
  data = almp_nma_model_nine_data,
  family = gaussian()
)
...
# Combine & flatten
almp_nma_model_nine_priors <- do.call(
  c,
  c(
    prior_base,
...
almp_nma_model_nine <- brm(
  formula = almp_nma_model_nine_formula,
  data = almp_nma_model_nine_data,
  prior = almp_nma_model_nine_priors,
  family = gaussian(),
  chains = 2,
  iter = 3000,
  warmup = 1500,
...
# inspect the results
summary(almp_nma_model_nine)

# inspect diagnostic plots
#plot(almp_nma_model_nine)

pp_check(almp_nma_model_nine, type = "dens_overlay", ndraws = 200)
pp_check(almp_nma_model_nine, type = "intervals")

# examine influence
almp_nma_model_nine_loo <- add_criterion(
  almp_nma_model_nine,
  "loo",
  reloo = FALSE
)
summary(almp_nma_model_nine_loo$criteria$loo) # check Pareto k
...
almp_nma_model_nine_tau_draws <- almp_nma_model_nine |>
...
almp_nma_model_nine_component_draws <- almp_nma_model_nine |>
  gather_draws(
    `b_.*:comp_.*`,
    regex = TRUE
  ) |>
...
almp_nma_model_nine_component_summary <- almp_nma_model_nine_component_draws |>
  group_by(
    outcome,
    component
  ) |>
...
saveRDS(
  almp_nma_model_nine_component_summary,
  "./visualisation/inputs/prototype_models/almp_nma_model_nine_component_summary.RDS"
)

saveRDS(
  almp_nma_model_nine_component_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_nine_component_draws.RDS"
)

saveRDS(
  almp_nma_model_nine_tau_draws,
  "./visualisation/inputs/prototype_models/almp_nma_model_nine_tau_draws.RDS"
)
